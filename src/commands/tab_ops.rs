...
pub fn reload_all_tabs_no_preserve(app_state: &mut AppState, curr_path: &Path) -> io::Result<()> {
    let mut map = HashMap::new();
    {
        let mut display_options = app_state.config.display_options.clone();
        display_options.preserve_selection = false;
